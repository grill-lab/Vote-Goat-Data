...
def scrape_tmdb(start, stop, key, mode, append):
    ...
    for i in range(start, (stop + 1)):
        ...
        if (current_target == None):
            skipped_count += 1
            skipped_items.append(current_target)
    ...
    write_failures_to_disk(skipped_items)
    pbar.finish() #Once we've complete the scraping, end the progress bar.

if __name__ == '__main__':
    scrape_mode = "movie" # Mode can be movie, people, etc..
    scrape_start = int(sys.argv[1]) # Start ID
    scrape_stop = int(sys.argv[2]) # End ID
    s_diff = scrape_stop - scrape_start
    API_KEY = 'PRIVATE_KEY' # Private key, don't share publicly

    time_before = time.time()
    scrape_tmdb(scrape_start, scrape_stop, API_KEY, scrape_mode, "credits,keywords") # Perform the scraping!
